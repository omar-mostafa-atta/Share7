...
public static class TestData
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Written the old way, into the typed tables, so the engine's backfill gives the path its
        // nodes — as the migration and the seeder do. The structure writer, the publisher and the
        // node reads all start from the node.
        await EngineTest.BackfillAsync(context, cancellationToken);
