...
import { ArrowLeft, ArrowRight, Check, ChevronDown, Copy, Search, X } from 'lucide-react'
...
/**
 * The bar under a paged list: where you are on the left, round page buttons
 * beside it, and an "Items Per Page" menu on the right.
 *
 * Renders nothing when the list fits in the smallest page size, since no page
 * size would split it. It stays up once a larger size has swallowed the list
 * onto one page — otherwise the menu that did it would vanish with no way back.
 *
 * Page numbers are windowed (`1 2 3 … 293 294`), so a list of four thousand
 * events does not draw four hundred buttons.
 */
export function Pagination({
  ...
  /** What is being counted, as in "Page 1 of 4 (38 accounts)". */
  noun?: string
}) {
  ...
  return (
    <nav className="s7-pager" aria-label="Pagination">
      <div className="s7-pager-main">
        <span className="s7-pager-summary">
          Page {page.toLocaleString()} of {pageCount.toLocaleString()} ({total.toLocaleString()} {noun})
        </span>

        <div className="s7-pager-pages">
          <button
            type="button"
            className="s7-pager-btn"
            disabled={page <= 1}
          ...
          {pageWindow(page, pageCount).map((item, i) =>
            item === null ? (
              <span key={`gap-${i}`} className="s7-pager-btn s7-pager-gap" aria-hidden>
                …
              </span>
            ) : (
              <button
                key={item}
                type="button"
                className={`s7-pager-btn ${item === page ? 'is-current' : ''}`}
      ...
      <label className="s7-pager-size">
        <span>Items Per Page</span>
        <span className="s7-pager-select">
...
/**
 * Page numbers to draw, with `null` for an elided run: the first and last page,
 * the current page and its neighbours, and a little more at whichever end the
 * admin is at — `1 2 3 … 293 294` on page 1, `1 … 149 150 151 … 294` mid-way.
 */
function pageWindow(page: number, pageCount: number): (number | null)[] {
  if (pageCount <= 7) return Array.from({ length: pageCount }, (_, i) => i + 1)

  const shown = new Set([1, pageCount, page - 1, page, page + 1])
  if (page <= 2) [2, 3, pageCount - 1].forEach((n) => shown.add(n))
  if (page >= pageCount - 1) [2, pageCount - 2, pageCount - 1].forEach((n) => shown.add(n))
  ...
  pages.forEach((n, i) => {
    ...
    // An ellipsis standing in for a single page takes as much room as the page
    // itself and hides it for nothing, so that one is drawn as a number.
    if (gap === 2) items.push(n - 1)
