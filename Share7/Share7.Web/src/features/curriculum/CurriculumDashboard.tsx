...
import { Meter, Note, Pagination, Segmented } from '../../components/ui/bits'
...
function IssueGroup({
  ...
  // Paged rather than capped. This used to show the first fifty and say "…and N more", with no way
  // to reach the rest short of fixing the first fifty.
  const paging = usePaging(issues.length)
  const shown = issues.slice(paging.start, paging.end)
