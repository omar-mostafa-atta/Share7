...
export function blockingReason(blueprint: Blueprint): string | null {
  if (blueprint.placeholderLines > 0) {
    return `${blueprint.placeholderLines} of its lines still name a lesson rather than a competency. A lesson cannot say what an examination covers — the content team replaces those stand-ins in the Content Studio.`
  }
