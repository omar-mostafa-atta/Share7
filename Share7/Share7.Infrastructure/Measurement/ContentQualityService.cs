...
public class ContentQualityService : IContentQualityService
{
    ...
    /// <summary>
    /// The items this surface is about: everything except the recovery pool. Recovery questions
    /// joined the item bank in the engine rebuild, but nothing answers them through the attempt
    /// endpoint, so they carry no evidence and are deliberately mapped to no learning target —
    /// counting them would report every one as "unmeasurable" content to fix.
    /// </summary>
    private IQueryable<Domain.Content.Item> MeasuredItems =>
        _dbContext.Items.Where(i =>
            !_dbContext.NodeItemMappings.Any(m => m.ItemId == i.Id && m.Role == Domain.Content.NodeItemRole.Recovery)
            || _dbContext.NodeItemMappings.Any(m => m.ItemId == i.Id && m.Role != Domain.Content.NodeItemRole.Recovery));
    ...
    {
        var items = await MeasuredItems.CountAsync(cancellationToken);
        var anchors = await MeasuredItems.CountAsync(i => i.IsAnchor, cancellationToken);

        var unmapped = await MeasuredItems
    ...
    {
        var query = _dbContext.ItemVersions.AsNoTracking()
            .Where(v => v.RetiredAtUtc == null && MeasuredItems.Any(i => i.Id == v.ItemId));
