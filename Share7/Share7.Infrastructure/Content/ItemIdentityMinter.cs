...
public class ItemIdentityMinter : IItemIdentityMinter
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await EnsureTargetMappingAsync(lessonId, item.Id, isNew, nowUtc, cancellationToken);

        return version;
    }

    public Task EnsureLessonTargetAsync(
        Guid lessonId, Guid itemId, bool itemIsNew, DateTime nowUtc, CancellationToken cancellationToken = default) =>
        EnsureTargetMappingAsync(lessonId, itemId, itemIsNew, nowUtc, cancellationToken);
    ...
    private async Task EnsureTargetMappingAsync(
        Guid lessonId, Guid itemId, bool itemIsNew, DateTime nowUtc, CancellationToken cancellationToken)
    {
        var target = await ResolveLessonTargetAsync(lessonId, nowUtc, cancellationToken);

        var exists = !itemIsNew
            && (_dbContext.ItemTargetMappings.Local.Any(
                    m => m.ItemId == itemId && m.TargetId == target.Id)
                || await _dbContext.ItemTargetMappings.AnyAsync(
                    m => m.ItemId == itemId && m.TargetId == target.Id, cancellationToken));

        if (exists) return;

        var hasPrimary = !itemIsNew
            && (_dbContext.ItemTargetMappings.Local.Any(m => m.ItemId == itemId && m.IsPrimary)
                || await _dbContext.ItemTargetMappings.AnyAsync(
                    m => m.ItemId == itemId && m.IsPrimary, cancellationToken));
