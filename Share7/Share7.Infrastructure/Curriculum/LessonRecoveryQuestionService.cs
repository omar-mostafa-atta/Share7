...
public class LessonRecoveryQuestionService : ILessonRecoveryQuestionService
{
    ...
    public LessonRecoveryQuestionService(ApplicationDbContext dbContext, ILanguageService languageService)
    {
    ...
    public async Task<LessonVersionDto?> GetVersionAsync(Guid lessonId, CancellationToken cancellationToken = default)
    {
        ...
        return await _dbContext.Lessons
            ...
            .Select(l => new LessonVersionDto
            {
                LessonId = l.Id,
                LangId = langId,
                // No recovery-set row means nothing has been uploaded in this language: version 0.
    ...
    public async Task<LessonQuestionsDto?> GetQuestionsAsync(Guid lessonId, CancellationToken cancellationToken = default)
    {
        ...
        // An empty list with version 0 is the "no recovery pool in this language yet" case — the
        // lesson exists and may well be playable, it just has no recovery sheet uploaded.
