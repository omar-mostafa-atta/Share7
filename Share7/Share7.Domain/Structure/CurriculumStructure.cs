...
/// <para>
/// <b>These rows are the source of truth for the curriculum's structure</b>, keeping the exact
/// GUIDs of the legacy typed rows they started as a projection of. Every structural write goes to
/// the node first and to the typed row in the same transaction, as a compatibility copy for the
/// readers not yet moved (and for the foreign keys student profiles and progress still hold). The
/// two cannot disagree: they are the same identifiers, written together. The typed tables are
/// dropped or turned into views at cutover. <see cref="CurriculumVersion.IsAuthoritative"/> is
/// set on the migrated Egyptian version to say so.
/// </para>
/// </summary>
public class CurriculumNode
{
    ...
    /// <summary>
    /// Set when the node is retired: hidden from students, its history kept. **Never deleted** —
    /// responses, mappings, progress and unlocks name it. Retiring a node retires every live node
    /// under it at the same instant, and restoring brings back exactly the ones retired with it.
    /// </summary>
    public DateTime? RetiredAtUtc { get; set; }

    /// <summary>
    /// Goes up by one on every change to the node itself — a rename, a move, a new position, a
    /// retirement or a restore. A Studio draft records the revision it started from, which is how a
    /// change made underneath it is noticed instead of silently overwritten.
    /// </summary>
    public int Revision { get; set; } = 1;

    public DateTime? UpdatedAtUtc { get; set; }

    /// <summary>Who made the last change. Null for the platform itself (backfills, seeding).</summary>
